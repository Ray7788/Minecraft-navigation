...
from mineclip import build_GPT, build_ViT, build_logit_scale, build_adapter, build_sequence_encoder
from mineclip import CrossEn, AllGather
from build import videoCLIP
...
class new_CLIP(videoCLIP):
    def __init__(self,
        ...
        super().__init__()
        self.image_encoder = build_ViT(pretrained_clip)
        self.text_encoder = build_GPT(pretrained_clip)
        self.sigmoid = torch.nn.Sigmoid()
        self.text_flow = [[self.text_encoder]]
        self.video_flow = [[self.image_encoder]]
    ...
    def get_text_embedding(self, text, entity_mask=None, action_mask=None):
        ...
        text_embedding = self.text_encoder.get_hidden_state(text, full=True)
