...
class NNActionSpaceWrapper(gym.Wrapper):
    ...
    The new action space is a MultiDiscrete space with 8 dimensions:
    - 0: forward and back, 0: noop, 1: forward, 2: back
    - 1: left and right, 0: noop, 1: left, 2: right
    - 2: jump, sneak, sprint, 0: noop, 1: jump, 2: sneak, 3: sprint
    - 3: camera pitch, 0: -180, n_pitch_bins - 1: +180
    - 4: camera yaw, 0: -180, n_yaw_bins - 1: +180,
    - 5: functional actions, 0: no_op, 1: use, 2: drop, 3: attack 4: craft 5: equip 6: place 7: destroy
    - 6: arg for "craft"
    - 7: arg for "equip", "place", and "destroy"

    Args:
        env (MineDojoSim): the environment to wrap
        discretized_camera_interval (Union[int, float]): the interval to discretize the camera angle
        strict_check (bool): whether to raise error when the wrapper detects an invalid action
    """
    ...
    def __init__(
        ...
        # check if the env is compatible with this wrapper
        assert (
        ...
        # transform action space to a new space
