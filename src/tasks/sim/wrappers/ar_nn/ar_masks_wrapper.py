...
class ARMasksWrapper(gym.ObservationWrapper):
    """
    Wrapper for the AR-NN agent.

    This wrapper adds a `masks` key to the observation space, which contains
    masks for the following:
        - action_type: a mask for the validity of each action category
        - action_arg: a mask for the validity of each action argument
        - equip: a mask for the validity of each equip action
        - place: a mask for the validity of each place action
        - destroy: a mask for the validity of each destroy action
        - craft_smelt: a mask for the validity of each craft/smelt action

    The action_type and action_arg masks are determined by the `action_categories_and_num_args`
    argument. The equip, place, destroy, and craft_smelt masks are determined by the inventory
    and nearby tools.

    Args:
        env (Union[MineDojoSim, gym.Wrapper]): The environment to wrap.
        action_categories_and_num_args (Optional[dict[str, int]], optional): A dictionary
            mapping action categories to the number of arguments that action category takes.
            Defaults to None, which uses the default action categories and number of arguments.
    """
    def __init__(
        self,
