...
class CMDExecutor:
    ...
    def execute_cmd(self, cmd: str, action: Optional[dict] = None):
        """
        Execute a MineCraft native command
        """
        # Check if the command starts with a slash and is a valid command
    ...
    def spawn_mobs(
        ...
        """
        Spawn mobs at relative positions

        Args:
            mobs (Union[str, List[str]]): Mob type(s)
            rel_positions (Union[np.ndarray, list]): Relative positions, is a list of [x, y, z]
            action (Optional[dict], optional): Action dict. Defaults to None.
        """
    ...
    def set_block(
        ...
        """
        Set block at relative positions
        https://minecraft.fandom.com/wiki/Commands/setblock#Arguments
        """
        if isinstance(blocks, str):
            blocks = [blocks]   # transfer to list
        if isinstance(rel_positions, list):
            rel_positions = np.array(rel_positions) # transfer to np.array
        if rel_positions.ndim == 1:
            rel_positions = rel_positions[np.newaxis, ...]
    ...
    def clear_inventory(self, action: Optional[dict] = None):
        """
        Clear inventory
        https://minecraft.fandom.com/wiki/Commands/clear#Syntax
        """
    ...
    def set_inventory(
        ...
        """
        Set inventory items
        https://minecraft.fandom.com/wiki/Commands/replaceitem#Syntax
        """
        obs, info = self._world.prev_obs, self._world.prev_info
...
# TODO:= summarize the following functions and try to integrate them 
